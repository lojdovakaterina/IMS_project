...
# Vytvořte funkci pro výpočet průměrů z jednoho souboru
def calculate_avgs(file_path, filename):
    with open(file_path, 'r') as file:
        data = file.read()

    # Rozdělte vstup na záznamy
    records = re.split(r'(?<=\bBASE MODEL\b)', data)

    # Inicializujte proměnné pro průměr
    total_iw = 0
    total_nw = 0
    ...
    # Pro každý záznam vypočtěte průměr
    for record in records:
        ...
        lines = record.split('\n')
        # Check for insufficient lines
        if len(lines) < 3:
            print(f"Skipping record due to insufficient lines: {lines}")
            continue
